...
abalone_clean <- abalone  |>
  # cleaning names so there are no spaces
  clean_names()  |>
...
# base layer: ggplot
ggplot(data = abalone_clean, # adding data to ggplot
       aes(x = mean_ph, # x-axis: pH, y-axis: change in shell SA
           y = change_in_area)) +
  # creating 2nd layer: points for data
  geom_point(size = 4,
             stroke = 1,
             fill = "firebrick4",
             shape = 21)
...
**The missing value comes from one pair of tanks that had a malfunction in the pH system**  
...
At pH = 8, the model predicts change in abalone shell SA to be 5.36/day (mm^2^ d^-1^). 
